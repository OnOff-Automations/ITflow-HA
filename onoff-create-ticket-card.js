class OnOffCreateTicketCard extends HTMLElement {
  ...
  set hass(hass) {
    ...
    if (!this.rendered) {
      ...
      // Reload tickets when hass updates
      this.loadTickets();
    }
  }
  ...
  attachEventListeners() {
    ...
    tabs.forEach(tab => {
      tab.addEventListener('click', () => {
        ...
        this.render();
        // Load tickets when switching to reply tab
        if (this.activeTab === 'reply') {
          setTimeout(() => this.loadTickets(), 100);
        }
      });
    });
...
${details}`;
    ...
    try {
      await this._hass.callService('onoff_itflow', 'create_ticket', {
